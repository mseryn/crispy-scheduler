...
def test_initialized_name():
    test_meeting = schedule.task.meeting(name = "Initialized With Name")
...
def test_initialized_time():
    test_meeting = schedule.task.meeting(time = datetime.datetime.now())
...
def test_initialized_room():
    ...
    test_meeting = schedule.task.meeting(room = test_room)
...
# Testing _participants manipulation: def test_default_participants(): test_meeting = schedule.task.meeting() assert_true((test_meeting.get_participants() == []), \
...
def test_single_initialized_participants():
    ...
    test_meeting = schedule.task.meeting(participants = [test_person])
...
def test_multiple_initialized_participants():
    ...
    test_meeting = schedule.task.meeting(participants = [test_person1, test_person2])
...
def test_set_participants():
    ...
    test_person2 = schedule.resource.person()
    test_meeting = schedule.task.meeting()
    test_meeting.overwrite_participants([test_person1, test_person2])
    assert_true((test_meeting.get_participants() == [test_person1, test_person2]), \
        "overwrite participants for meeting not correctly set")

def test_clear_participants():
    test_person = schedule.resource.person()
    test_meeting = schedule.task.meeting(participants = [test_person])
    test_meeting.clear_participants()
    assert_true((test_meeting.get_participants() == []), \
        "clear participants for meeting not correctly set")

def test_append_participants():
    test_person1 = schedule.resource.person()
    test_person2 = schedule.resource.person()
    test_meeting = schedule.task.meeting(participants = [test_person1])
    test_meeting.add_participants([test_person2])
    assert_true((test_meeting.get_participants() == [test_person1, test_person2]), \
        "appending participant for meeting not correctly set")
